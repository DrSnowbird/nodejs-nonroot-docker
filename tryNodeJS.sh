...
echo
docker run -d --rm --name ${instanceName} -v $PWD/data:/data -p ${HOST_PORT}:${SERVER_PORT} --workdir /data openkbs/nodejs-nonroot-docker node /data/${SCRIPT_FILE}
